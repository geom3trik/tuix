...
//     type Data<'a> = <W as Widget>::Data<'a>;
...
//     pub fn bind<L: Lens, F>(self, lens: L) -> Wrapper<L, Self> 
//     {
//         Wrapper::new(self, lens)
//     }
// }

// impl<T: std::fmt::Debug + Any, W: Widget> Widget for ListView<T, W> {
//     type Ret = Entity;
//     type Data<'a> = &'a Vec<T>;
//     fn on_build(&mut self, state: &mut State, entity: Entity) -> Self::Ret {
//         state.focused = entity;
...
//     fn on_update<'a>(&mut self, state: &mut State, entity: Entity, data: &&'a Vec<T>) {
...
//             if let Some(mut event_handler) = state.event_handlers.remove(&child) {
//                 event_handler.on_update(state, child, (&data[index]).into());
