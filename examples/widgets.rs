...
fn main() {
    Application::new(WindowDescription::new().with_min_inner_size(300, 300), |state, window| {
        ...
        // BUTTONS PANEL
        let panel =
            Panel::new("Buttons").build(state, scroll, |builder| builder.set_width(Stretch(1.0)));

        panel
            .set_child_space(state, Pixels(10.0))
            .set_child_between(state, Pixels(10.0));

        // // BUTTON
        let row = Row::new().build(state, panel, |builder| builder);
        Label::new("Button").build(state, row, |builder| {
            builder.set_height(Pixels(30.0)).set_width(Pixels(120.0))
        });
        ...
        let list = List::new().build(state, row, |builder| {
            builder.set_width(Stretch(1.0)).set_height(Auto)
        });
        ...
        let list = List::new().set_multi().build(state, row, |builder| {
            builder.set_width(Stretch(1.0)).set_height(Auto)
        });
        CheckButton::new(true).build(state, list, |builder| {
            builder
        ...
        let (_, _, popup) = Dropdown::new("Dropdown")
            .set_multi()
            .build(state, row, |builder| builder.set_height(Pixels(30.0)));
        let list = List::new()
            .set_multi()
        ...
        Slider::new()
            ...
            .build(state, row, |builder| {
                builder.set_top(Stretch(1.0)).set_bottom(Stretch(1.0))
            });
