...
///
/// The IdManager is generic on ID type, requiring only that the ID type implments [GenerationalId]
...
impl <I: GenerationalId + Copy> IdManager<I> {
    ...
    pub(crate) fn create(&mut self) -> I {
    ...
    /// ``` 
    pub(crate) fn destroy(&mut self, id: I) -> bool {
        if self.is_alive(id) {
            let index = id.index();
    ...
    pub(crate) fn is_alive(&self, id: I) -> bool {
