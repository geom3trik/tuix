...
pub(crate) struct AnimatableSet<T: Interpolator> {
    /// Mapping from entity index to data index and animation index
    //entity_indices: Vec<Index>,
    // /// Mapping from rule index to shared data index
    // rule_indices: Vec<usize>,
    /// Mapping from animation index to animation states index
...
where
    T: 'static + Default + Clone + Interpolator
{
    ...
    fn insert_animation(&mut self, animation: Animation, animation_description: AnimationState<T>) {
        self.animations.insert(animation, animation_description);
    }

    fn remove_animation(&mut self, animation: Animation) -> Option<AnimationState<T>> {
        self.animations.remove(animation)
    }

    fn insert_rule(&mut self, rule: Rule, value: T) {
        self.shared_data.insert(rule, value);
    }

    fn remove_rule(&mut self, rule: Rule) -> Option<T> {
        self.shared_data.remove(rule)
    }

    /// Inserts a transition for a given rule
    ///
    /// Trnasitions are animations which are defined for a particular rule. When an entity is linked to
    /// a rule any transition associated with that rule will play for that entity.
    ///
    fn insert_transition(&mut self, rule: Rule, animation_description: AnimationState<T>) {

    }

    fn play_animation(&mut self, entity: Entity, animation: Animation) {
    ...
    fn tick(&self, time: std::time::Instant) {
    ...
    fn link(&mut self, entity: Entity, rules: &[Rule]) {
        ...
        for rule in rules.iter() {
            if let Some(shared_data_index) = self.shared_data.dense_idx(*rule) {
                ...
                let anim_index = shared_data_index.anim_index as usize;

                if anim_index < self.animations.dense.len() {
                    let mut anim_state = self.animations.dense[anim_index].value.clone();
                    // TODO
                    // Check if the entity is already being animated
                    // If it's being animated by a regular animation, don't transition
                    // If it's being animated by a trnasition, then blend the trnasition to the new one
                }
