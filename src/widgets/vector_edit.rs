...
use crate::{MouseButton, State, Propagation};

use crate::widgets::{Button, Textbox, Dropdown, Item, DropdownEvent, TextboxEvent};
...
impl BuildHandler for Dimension {
    ...
    fn on_build(&mut self, state: &mut State, entity: Entity) -> Self::Ret {
        ...
        entity.set_text(state, &self.text).set_text_justify(state, Justify::Center).set_text_align(state, Align::Center);
...
impl<T> VectorEdit<T> 
    where T: 'static + Default + std::fmt::Debug + std::fmt::Display + Copy + PartialEq + std::str::FromStr
{
    pub fn new() -> Self {
        VectorEdit {
            x: Entity::null(),
...
where T: 'static + Default + std::fmt::Debug + std::fmt::Display + Copy + PartialEq + std::str::FromStr
{
    ...
    fn on_build(&mut self, state: &mut State, entity: Entity) -> Self::Ret {
        ...
        self.x = Textbox::new(&self.xval.to_string()).build(state, entity, |builder| builder.set_flex_grow(1.0));
        self.y = Textbox::new(&self.yval.to_string()).build(state, entity, |builder| builder.set_flex_grow(1.0).set_margin_left(Length::Pixels(5.0)));
        self.z = Textbox::new(&self.zval.to_string()).build(state, entity, |builder| builder.set_flex_grow(1.0).set_margin_left(Length::Pixels(5.0)));
        self.w = Textbox::new(&self.wval.to_string()).build(state, entity, |builder| builder.set_flex_grow(1.0).set_margin_left(Length::Pixels(5.0)));
        ...
        self.reveal = state
            .style
            .flex_grow
            .insert_animation(AnimationState::new()
                .with_duration(std::time::Duration::from_millis(100))
                .with_keyframe((0.0, 0.0))
                .with_keyframe((1.0, 1.0))
            );

        self.grow = state.style.margin_left.insert_animation(AnimationState::new()
            .with_duration(std::time::Duration::from_millis(100))
        ...
        self.shrink = state.style.margin_left.insert_animation(AnimationState::new()
            .with_duration(std::time::Duration::from_millis(100))
                ...
                DropdownEvent::SetText(text, _) => {
                    match text.as_ref() {
                        "1" => {
                            ...
                            state.insert_event(Event::new(VectorEditEvent::Dim1(self.xval)).target(entity));
                        }
                        ...
                        "2" => {
                            ...
                            state.insert_event(Event::new(VectorEditEvent::Dim2(self.xval, self.yval)).target(entity));
                        }
                        ...
                        "3" => {
                            ...
                            state.insert_event(Event::new(VectorEditEvent::Dim3(self.xval, self.yval, self.zval)).target(entity));
                        }
                        ...
                        "4" => {
                            ...
                            state.insert_event(Event::new(VectorEditEvent::Dim4(self.xval, self.yval, self.zval, self.wval)).target(entity));
                        }
                ...
                TextboxEvent::ValueChanged(text) => {
                    if let Ok(val) = text.clone().parse::<T>() {
                        ...
                        match self.num_of_dims {
                            1 => state.insert_event(Event::new(VectorEditEvent::Dim1(self.xval)).target(entity)),
                            2 => state.insert_event(Event::new(VectorEditEvent::Dim2(self.xval, self.yval)).target(entity)),
                            3 => state.insert_event(Event::new(VectorEditEvent::Dim3(self.xval, self.yval, self.zval)).target(entity)),
                            4 => state.insert_event(Event::new(VectorEditEvent::Dim4(self.xval, self.yval, self.zval, self.wval)).target(entity)),
                            _=> {}
                        }
